...
func setupFlags(cmd *cobra.Command) error {
	...
	cmd.Flags().String("acl-model-file", "", "Path to ACL mode.")
...
func (c *cli) setupConfig(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("bootstrap: %v\n", viper.GetBool("bootstrap"))
	fmt.Printf("start-join-addrs: %v\n", viper.GetStringSlice("start-join-addrs"))
	...
	c.cfg.PeerTLSConfig.CertFile = viper.GetString("peerr-tls-cert-file")
	...
	if c.cfg.ServerTLSConfig.CertFile != "" && c.cfg.ServerTLSConfig.KeyFile != "" {
		...
		if err != nil {
			fmt.Println("Certs server FAILED")
			return err
		}
	}
	...
	if c.cfg.PeerTLSConfig.CertFile != "" && c.cfg.PeerTLSConfig.KeyFile != "" {
		...
		if err != nil {
			fmt.Println("Certs peer FAILED")
			return err
		}
	}
	fmt.Println("Certs fine")
